...
Iter1 = 10 #Symmetric matrix of cells and steps
n = 8 #number of bits
firstrow = np.random.randint(2, size=(Iter1 + 1)) #Initial condition
...
for item in Matrix:
    P = []
    P = np.array(list(item), dtype=int)
    print("Count: ", count, "/", pow(2, 8))
...
#print((P))
    ...
    for i in range(0,Iter1):
        ...
        for j in range(0,(len(Z2))):
            if j == 0: #if first
                ...
                num2 = Z2[j]
                num3 = Z2[j+1]
            hood = np.array(([num1,num2,num3]), dtype=int)
    ...
    input_matrices = []
    input_matrices.append(np.array(Z3))
    ...
    outfile = open(path + 'entropypattern.csv','a')
    outfile.write(str((P)) + ',' + str(profile(matrices)) + '\n')
